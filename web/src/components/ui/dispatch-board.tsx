...
import { TASKS, memberById, formatDeadline, Priority, OpsTask } from "@/lib/mock";
...
function TaskCard({ task }: { task: OpsTask }) {
  ...
  return (
    <div
      className={clsx(
        "ops-surface cursor-pointer",
        "p-[10px]",
        task.aiClassified ? "ai-classified-card" : "hover:bg-surface-raised transition-colors"
      )}
    >
      ...
      <div className="flex items-center justify-between pt-1.5 border-t border-divider">
        ...
        <div className="text-right">
          <div className="mono text-xs text-text-primary tabular">{d.time}</div>
